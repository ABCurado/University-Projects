...
int main(int argc, char *argv[] ){
	...
	for (i = 0; i < PROCESSES; ++i) {
		...
		if((pid=fork())<0){
			...
			int randAccount = (rand() % max + min);
			max = 150;
			min = 1;
			srand(time(NULL)+getpid()+i+randAccount);
			int randMoney = (rand() % max + min);
			message->id = randAccount;
			message->money = randMoney;
			message->remov = 1;
			...
			printf("Foram removidos %.2f euros da conta %d.\n", message->money, message->id);
