...
//    }
    ...
    @Test
    public void ensureAccountLoadWorksHasIntended() throws Exception {
        final Set<RoleType> roles = new HashSet<RoleType>();
        roles.add(RoleType.Admin);
        final SystemUser aSystemUser = new SystemUser("userName", "password", "firsName", "lastName", this.anEmail,
                roles);
        ...
        aCafeteriaUser.registerLoad(BigDecimal.ONE);
        assertEquals(new Money(1.0, Currency.getInstance("EUR")), aCafeteriaUser.accountBalance());
    }
    
    
    @Test
    public void ensureAccountDebtWorksHasIntended() throws Exception {
        final Set<RoleType> roles = new HashSet<RoleType>();
        roles.add(RoleType.Admin);
        final SystemUser aSystemUser = new SystemUser("userName", "password", "firsName", "lastName", this.anEmail,
                roles);
        ...
        final OrganicUnit anOrganicUnit = new OrganicUnit("acronymA", "nameA", "descriptionA");
        final CafeteriaUser aCafeteriaUser = new CafeteriaUser(aSystemUser, anOrganicUnit, aMecanographicNumber);
        aCafeteriaUser.registerLoad(BigDecimal.TEN);
        aCafeteriaUser.registerExpense(BigDecimal.ONE);
        assertEquals(new Money(9.0, Currency.getInstance("EUR")), aCafeteriaUser.accountBalance());
    }
}
