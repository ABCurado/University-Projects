...
class HttpRequest implements Runnable {
    ...
    public HttpRequest(Socket cli_s) throws Exception {
        s = cli_s;
    }
        ...
        do {	// FIRSH HEADER
            ...
            if (line.startsWith("Content-Length: ")) {
                len = Integer.parseInt(line.split(" ")[1]);
                    ...
                    startsWith("Content-Type: multipart/form-data; boundary=")) {
    ...
    void replyPostList() {
        String s1 = "<html>"
                ...
                +   "setInterval(doStuff, timeoutPeriod);" 
                + "}"
        ...
        ServerHTTP.updateList();

        String list = "";

        for (String data : ServerHTTP.list) {
            String[] info = data.split(";");
            list = list + "<li><a href=http://" + info[1] + ":" + ServerHTTP.port + "/files/" + info[2] + ">" + info[0] + " - " + info[2] + "</a>";
        }

        writeLineCRLF("HTTP/1.0 200 OK");
        writeLineCRLF("Content-type: text/html");
        int len = s1.length() + s2.length() + list.length();
        writeLineCRLF("Content-length: " + len);
        writeLineCRLF("Connection: close");
        writeLineCRLF("");
        try {
