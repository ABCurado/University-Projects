...
public class UdpServer extends Server {
    ...
    protected void boot(int port) {
        try {
            ...
            this.server = new UdpSocket(port);
    ...
    private synchronized Action getActionFromRequest(String request, String message, Map<String, Object> arguments) {
        ...
        arguments.put("volt-username", System.getProperty("user.name"));
    ...
    public void stream(int port) {
        ...
        while (isActive()) {
            try {
                ...
                final UdpPacket request = new UdpPacket(new byte[512], 512);

                this.server().receive(request.packet());
    ...
    protected void protocol(String route, String target, int parts, String message)
    {
        ...
        if (parts > 0) {
            ...
            for (String part : messages) {
                try {
                    UdpPacket packet = new UdpPacket(part.getBytes("UTF-8"), part.length(), address, port);

                    this.server().send(packet.packet());
            ...
            try {
                ...
                UdpPacket packet = new UdpPacket(packetMessage.getBytes("UTF-8"), packetMessage.length(), address, port);
                this.server().send(packet.packet());
                this.server().setBroadcast(false);
